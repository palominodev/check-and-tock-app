...
export const CrearProductoForm = () => {
	...
	const onCreateProduct = async(e:any) => {
		setIsLoading(true)
		e.preventDefault()		
		const {name_product, price_product,categoria_product, cantidad_minima} = Object.fromEntries(new FormData(e.target))	
		toast({
  ...
  return (
	<form onSubmit={(e) => onCreateProduct(e)}>
		...
		<Button disabled={isLoading} className="bg-green-600 px-5 py-3 font-bold text-white rounded-lg" type="submit">Agregar</Button>
	</form>
  )
}